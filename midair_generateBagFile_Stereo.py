...
def main(*args):
    ...
    if camera == "left_and_right":
        camera_temp = "color_left"
    else:
        camera_temp = camera
    if (not any('.JPEG' in a for a in os.listdir(sensorRecordsPath + "/" + camera_temp + "/trajectory_" + trajectory))) and any('.zip' in a for a in os.listdir(sensorRecordsPath + "/" + camera_temp + "/trajectory_" + trajectory)):
    ...
    if camera != "left_and_right":
        ...
        imagePaths = list(f1['trajectory_' + trajectory]['camera_data']["color_left"])
        # save left camera frames in one rosbag
        for line in imagePaths:
            line = str(line)
            line = line[2:-1]
            absImg = sensorRecordsPath + "/" + line # get the absolute file path
            ...
            if (os.path.isfile(absImg)) and (os.stat(absImg).st_size != 0):
                ...
                # write image to the bag file under the 'cam0/image_raw' topic
                bag.write("camera/left/image_raw", img_msg, img_msg.header.stamp)
                ...
                imagePaths = list(f1['trajectory_' + trajectory]['camera_data']["left_and_right"])
        
        # save right camera frames in one rosbag
        imagePaths = list(f1['trajectory_' + trajectory]['camera_data']["color_right"])
        ...
        for line in imagePaths:
            ...
            if (os.path.isfile(absImg)) and (os.stat(absImg).st_size != 0):
                ...
                # write image to the bag file under the 'cam0/image_raw' topic
                bag.write("camera/right/image_raw", img_msg, img_msg.header.stamp)
